...
* Outputs :  phenotype_disagreements.wmf ( bar chart of method results by antibiotic)
*  phenotype_disagreements_withgold.wmf ( bar charts of method results by antibiotic and lab results, restricted to results where lab result either R or S)
* all_sens.wmf ( sensitivity of each method, by antibiotic) and all_spec.wmf (specificity of each method, by antibiotic)
...
|| scatter  sens newcount if method=="m", mcolor(blue)
...
|| scatter  spec newcount if method=="m", mcolor(blue)
